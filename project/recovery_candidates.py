...
def build_recovery_candidates(
    ...
    market_high_alert = any(alert.get("category") == "market" and alert.get("severity") == "high" for alert in alerts)

    if best_score >= 2.3 and supportive_backdrop:
        return _payload("build", "仕込み候補", "高重要度警告下の参考候補です。" if market_high_alert else "下落後の反転初期として監視したい候補があります。", best_asset, best_sector, market_high_alert=market_high_alert)
    if best_score >= 1.9:
        return _payload("watch", "先回り観察", "高重要度警告下の参考候補です。" if market_high_alert else "まだ初期段階ですが、弱い状態から改善し始めた候補があります。", best_asset, best_sector, market_high_alert=market_high_alert)
...
def _payload(
    ...
    *,
    ...
    if market_high_alert:
        rationale.append("高重要度の市場警告があるため、先回り候補は参考表示です。")
    return {
        "tier": tier,
        "label": label,
        "summary": summary,

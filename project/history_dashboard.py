            ...
            <div class="relation-row drivers">
              ...
              <div class="relation-group col-3">
                ...
                <div class="node-grid single two-rows">
                  ...
                  <button class="map-node alerts" data-node="japan_risk" type="button" aria-pressed="false"><div class="label">円建てリスク</div><div id="nodeJapanRisk" class="strong">-</div><div id="nodeJapanRiskSub" class="mini">USDJPY と円建て換算</div></button>
                </div>
              </div>
            </div>
  ...
  <script>
    ...
    function renderDetail(entry) {
      ...
      if (node === 'regime') {
        ...
        renderTable('市場レジームの補助情報', ['項目', '内容'], [['合成スコア', formatScore(entry.score)], ['上昇再開の証拠', `${entry.spot_signal.recovery_evidence.grade} / ${formatScore(entry.spot_signal.recovery_evidence.score)}`], ['騙し上昇の警戒', entry.spot_signal.blocker_assessment.level], ['先導セクター', entry.top_sector?.label || '-']]);
        return;
      }
      if (node === 'score') {
        ...
        renderBoxes([{ key: '合成スコア', value: formatScore(entry.score) }, { key: '上昇再開の証拠', value: `${entry.spot_signal.recovery_evidence.grade} / ${formatScore(entry.spot_signal.recovery_evidence.score)}` }, { key: 'legacy判定用', value: formatScore(entry.adjusted_score) }, { key: 'スポット判断', value: entry.spot_signal.label }]);
        setDetailCopy('合成スコアは押し目条件の要約です。ここでの内部警告件数は alerts/warnings の件数で、危険ライン段階とは別の判定です。危険ライン段階は下の表で独立して確認します。');
        renderBoxes([{ key: '合成スコア', value: formatScore(entry.score) }, { key: '上昇再開の証拠', value: `${entry.spot_signal.recovery_evidence.grade} / ${formatScore(entry.spot_signal.recovery_evidence.score)}` }, { key: '騙し上昇の警戒', value: entry.spot_signal.blocker_assessment.level }, { key: '危険ライン段階', value: formatRiskStageInline(entry.risk_lines) }]);
        renderTable('合成スコアの確認項目', ['項目', '内容'], [['市場レジーム', entry.regime.label], ['サイクル判定', entry.cycle.label], ['上昇再開の証拠', `${entry.spot_signal.recovery_evidence.grade} / ${formatScore(entry.spot_signal.recovery_evidence.score)}`], ['騙し上昇の警戒', `${entry.spot_signal.blocker_assessment.level} / ${entry.spot_signal.blocker_assessment.summary}`], ['危険ライン段階', formatRiskStageInline(entry.risk_lines)], ['危険ライン件数', emphasizeRiskText(`危険 ${entry.risk_lines?.danger_count || 0} / 非常に危険 ${entry.risk_lines?.extreme_count || 0}`)], ['内部警告件数', internalWarningLabel(entry)], ['内部警告と危険ラインの違い', emphasizeRiskText('内部警告は alerts/warnings の件数、危険ラインは market stress の段階判定です。')]]);
        return;
      }
      if (node === 'cycle') {
      ...
      if (node === 'spot') {
        ...
        renderBoxes([{ key: 'legacy判断', value: formatActionInline(entry.spot_signal.legacy_action, entry.spot_signal.legacy_label) }, { key: '新判断', value: formatActionInline(entry.spot_signal.action_decision.action, entry.spot_signal.action_decision.label) }, { key: '上昇再開の証拠', value: `${entry.spot_signal.recovery_evidence.grade} / ${formatScore(entry.spot_signal.recovery_evidence.score)}` }, { key: '騙し上昇の警戒', value: entry.spot_signal.blocker_assessment.level }]);
        renderTable('スポット判断の根拠', ['順番', '内容'], [[ '1', emphasizeRiskText(`上昇再開の証拠: ${entry.spot_signal.recovery_evidence.grade} / ${formatScore(entry.spot_signal.recovery_evidence.score)}`) ], [ '2', emphasizeRiskText(`騙し上昇の警戒: ${entry.spot_signal.blocker_assessment.level} / ${entry.spot_signal.blocker_assessment.summary}`) ], [ '3', emphasizeRiskText(`新判断: ${entry.spot_signal.action_decision.action} / mode ${entry.spot_signal.action_decision.mode}`) ], ...(entry.spot_signal.rationale || []).map((reason, idx) => [String(idx + 4), emphasizeRiskText(reason)])]);
        return;
      }
      if (node === 'sector') {
      ...
      if (node === 'japan_risk') {
        const japanRisk = entry.japan_risk || {};
        const usdJpy = japanRisk.usd_jpy || {};
        el('detailTitle').textContent = '円建て・為替リスク';
        el('detailSubtitle').textContent = 'USDJPY と外貨資産の円建て換算';
        setDetailCopy('円建て・為替リスクは、日本円で見た実質的な値動きと、外貨建て資産のリターンに為替がどれだけ寄与しているかを確認する補助ビューです。市場ストレス判定とは分けて確認します。');
        renderBoxes([
          { key: '判定', value: japanRisk.level_label || '-' },
          { key: 'USDJPY', value: usdJpy.current ?? '-' },
          { key: '4週変化', value: formatSigned(usdJpy.change_4w, 4) },
          { key: '為替シグナル', value: usdJpy.signal_label || '-' },
        ]);
        const rows = [
          ['要約', japanRisk.summary || '-'],
          ['フラグ', (japanRisk.flags || []).join(', ') || 'なし'],
        ];
        (japanRisk.foreign_assets || []).forEach((row) => {
          rows.push([
            `${row.asset_class || '-'} / ${row.ticker || '-'}`,
            `USD建て4週 ${formatSigned(row.usd_return_4w, 4)} / 円建て4週 ${formatSigned(row.jpy_return_4w, 4)} / 為替寄与 ${formatSigned(row.fx_contribution_4w, 4)} / ${row.signal_label || '-'}`,
          ]);
        });
        renderTable('円建てリスクの明細', ['項目', '内容'], rows);
        return;
      }
      if (node === 'asset') {
    ...
    function updateSummary(entry) {
      ...
      el('metricRegimeCopy').textContent = entry.data_reliability?.decision_allowed
        ? `上昇再開 ${entry.spot_signal.recovery_evidence.grade} / 警戒 ${entry.spot_signal.blocker_assessment.level} を踏まえて、現在の主導レジームを読む領域です。`
      ...
      el('metricSpotSub').innerHTML = entry.data_reliability?.decision_allowed
        ? emphasizeRiskText(`騙し上昇の警戒: ${entry.spot_signal.blocker_assessment.level} / 新判断 ${entry.spot_signal.action_decision.action}`)
      ...
      el('nodeSpotSub').innerHTML = entry.data_reliability?.decision_allowed
        ? emphasizeRiskText(`新判断 ${entry.spot_signal.action_decision.action} / 警戒 ${entry.spot_signal.blocker_assessment.level}`)
      ...
      el('nodeAlertsSub').innerHTML = highAlert
        ...
        : '内部警告は静穏';
      el('nodeJapanRisk').textContent = entry.japan_risk?.level_label || '-';
      el('nodeJapanRiskSub').textContent = entry.japan_risk?.usd_jpy?.signal_label || 'USDJPY と円建て換算';
...
def _normalize_dashboard_entry(data: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "spot_signal": {
            "key": data.get("spot_signal", {}).get("action_decision", {}).get("action", data.get("spot_signal", {}).get("action", "")),
            "label": ACTION_LABELS.get(
                data.get("spot_signal", {}).get("action_decision", {}).get("action", data.get("spot_signal", {}).get("action", "")),
                data.get("spot_signal", {}).get("action_decision", {}).get("action", data.get("spot_signal", {}).get("action", "")),
            ),
            "legacy_action": data.get("spot_signal", {}).get("legacy_action", data.get("spot_signal", {}).get("action", "")),
            "legacy_label": ACTION_LABELS.get(
                data.get("spot_signal", {}).get("legacy_action", data.get("spot_signal", {}).get("action", "")),
                data.get("spot_signal", {}).get("legacy_action", data.get("spot_signal", {}).get("action", "")),
            ),
            ...
            "action_decision": {
                ...
                "mode": data.get("spot_signal", {}).get("action_decision", {}).get("mode", "legacy"),
            },
        },
...
def _normalize_japan_risk(payload: dict[str, Any]) -> dict[str, Any]:
    level = str(payload.get("level", "unknown"))
    labels = {"high": "高", "moderate": "中", "low": "低", "unknown": "判定保留"}
    return {
        "available": bool(payload.get("available", False)),
        "level": level,
        "level_label": labels.get(level, level),
        "flags": list(payload.get("flags", [])),
        "summary": payload.get("summary", "-"),
        "usd_jpy": payload.get("usd_jpy", {}),
        "foreign_assets": list(payload.get("foreign_assets", [])),
    }

...
def test_build_regime_leading_candidates_keeps_candidates_under_high_market_alert():
    index = pd.date_range("2025-01-03", periods=24, freq="W-FRI")
    prices = pd.DataFrame(
        {
            "XLU": [100, 99, 98, 97, 96, 95, 94, 93, 92, 91, 90, 89, 89, 89, 90, 91, 92, 93, 94, 95, 95, 96, 97, 98],
    ...
    result = build_regime_leading_candidates(
        prices=prices,
        sector_map={"Utilities": "XLU", "Materials": "XLB"},
        region_map={"Japan": "EWJ", "Developed": "VEA"},
        asset_map={"Gold": "GLD"},
        sector_rotation={"table": [{"ticker": "XLU", "sector_name_ja": "公益事業", "return_12w": 0.01, "rank": 4, "rotation_phase": "improving", "rotation_phase_ja": "改善"}]},
        availability_map={ticker: {"status": "ok"} for ticker in ["XLU", "XLB", "EWJ", "GLD", "VEA"]},
        regime={"regime_label": "transition"},
        reliability={"decision_allowed": True},
    ...
    assert any("参考表示" in reason for reason in result["rationale"])

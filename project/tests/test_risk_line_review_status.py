...
def test_run_periodic_risk_line_maintenance_generates_proposal_when_due(monkeypatch):
    ...
    monkeypatch.setattr("project.risk_line_review_status.write_risk_line_recalibration_outputs", lambda config_path, sample_only=False: calls.__setitem__("recal", calls["recal"] + 1))

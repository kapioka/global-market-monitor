...
def build_regime_leading_candidates(
    ...
    if best_score >= 2.15:
        return _payload(
            "priority",
            "レジーム先回り候補",
            "高重要度警告下の参考候補です。" if market_high_alert else "次のレジームで効きやすい資産・地域・セクターの候補があります。",
            regime_label,
            best_sector,
            best_region,
            best_asset,
    ...
    if best_score >= 1.45:
        return _payload(
            "watch",
            "レジーム観察",
            "高重要度警告下の参考候補です。" if market_high_alert else "まだ先回り段階ですが、次の地合いで効きやすい候補があります。",
            regime_label,
            best_sector,
            best_region,
            best_asset,
...
def _payload(
    ...
    *,
    ...
    if market_high_alert:
        rationale.append("高重要度の市場警告があるため、レジーム先回り候補は参考表示です。")
    return {
        "tier": tier,
        "label": label,
        "summary": summary,

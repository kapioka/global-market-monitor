...
- CSVからSQLiteへの移行検証をpandas 2と3で統一し、通常の欠損値を保持しながら正負の無限値をfail-closedで拒否するようにしました。
- CIを依存lockと自己完結fixtureで再現可能にし、ローカル生成データや実行速度へ依存しないテストへ修正しました。
- CIの秘密情報検査からコンパイル済みPythonキャッシュとテスト生成物だけを除外し、追跡ソースの検査を維持したままバイナリ誤検出を防ぎました。

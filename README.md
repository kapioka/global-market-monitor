...
v0.10.0は、市場警戒年代記を通常実行から自動更新できるようにし、長期保存に向けたSQLite移行を安全な照合段階で開始するリリースです。

- 通常実行時に、検証済みの根拠データから市場警戒年代記を生成または更新します
- 補足ダッシュボードから年代記を別ウィンドウで開けます
- エピソードごとに、価格推移、当時の関連指標、根拠、評価、データの出所を確認できます
- 関連指標は当時の証拠から選び、ACWIを含め最大5系列を比較表示します
- 市場時系列をローカルSQLiteへ安全に初回移行し、既存CSVとの完全一致を確認しました
- Risk Engine V2の証拠整合性を改善し、欠落や不一致を安全側で停止するようにしました
...
年代記は過去の診断を読み返すための機能です。現在の`final_action`や買い候補度を変更しません。必要な根拠が不足、不整合、期限切れの場合は古い内容を最新として更新せず、利用できない理由を表示します。

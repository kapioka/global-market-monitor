...
v0.9.1 は、日本在住者向けにレポート表示の文言を整え、危険ラインの見え方を調整する小さな改善版です。売買判断ロジックを変更する版ではありません。

- レポート内に残っていた英語文や内部コード表記を日本語へ整理しました
- しきい値ルール認証、国内文脈、統合リスク文脈、資産候補、Hindenburg Omen 周辺の表示を読みやすくしました
- Hindenburg Omen の表示カード内に、手動取得方法を開くリンクを追加しました
- 中央の危険ライン表示を、現在値、危険度、注意ライン、危険ライン、非常に危険ラインの距離感が分かる棒グラフへ調整しました
- `final_action`、買い候補度、危険ライン判定ロジック、threshold JSON、Hindenburg Omen 判定は変更していません
